...
client.on("message", msg => {
 if(!db.has(`reklam_${msg.guild.id}`)) return;
        const reklam = [".com", ".net", ".xyz", ".tk", ".pw", ".io", ".me", ".gg", ".gl", ".org", ".com.tr", ".biz", "net", ".rf.gd", ".az", ".party", "discord.gg",];
...
.setThumbnail()
.addField(`**AloneRPV3 Betaya Hoşgeldiniz Kayıt Olmak İçin !kayit Yazmanız Yeterlidir \n @everyone**`)
.setTimestamp();
...
client.on("guildMemberAdd", member => {
  var kanal = client.channels.cache.get("782711012054532116")
  const embed = new Discord.MessageEmbed()
    .setColor("YELLOW")
    .setFooter(new Date().getTime() - member.user.createdAt.getTime() < 2592000000 ? " \:9330_tickred: /:no1: Bu Hesap Tehlikeli!" : "\:5845_tickgreen: :tik3: Bu Hesap Güvenli!")
    .setImage("https://cdn.discordapp.com/attachments/780112439160340490/780814924560990259/elfrpgif.gif")
    .addField(`Mesaj`, [`  

• **${member.user} Sunucumuza Hoş Geldin**     

• Seninle Beraber **${member.guild.memberCount}** Kişiyiz

►:tik3: -Deneyimli ve ilgili yetkili ekip!
►:tik3: -Yüksek düzeyde developer kadrosu!
►:tik3: -Özel amaçlar üzerine kullanılabilir discord botlarımız!
►:tik3: -Seçkin üye alımları!
►:tik3: -Güncel scriptler!
►:tik3: -ŞEHİR + KASABA SİSTEMLİ
►:tik3: -Gelişmiş log sistemi!
►:tik3: -Gelişmiş  ymapler!
►:tik3: -Gelişmiş şehir sistemi! 

• Sunucumuzun Sınırsız Davet Bağlantısı **https://discord.gg/uggs2YajJ7**

Bu hesap ${(new Date().getTime() - member.user.createdAt.getTime() ) / 108000 } Zaman Önce Kuruldu.
    `]);
  kanal.send(embed);
});
...
client.on('ready', ()=>{
client.channels.cache.get('779830604472123442').join()
})
...
client.on('message', async msg => {
  ...
  if(msg.content == `<@!763716184267948032>`) return msg.channel.send(`> **DLive | Prefix**\n\n>  **Sanırım beni etiketlediniz.**\n >  Buyurun prefix(ön ek)im \`${prefix}\``);
});
...
.setDescription(` <@!${message.author.id}> **adlı kullanıcı tarafından** <#${message.channel.id}> **kanalına gönderilen mesaj silindi!** \n\nSilinen Mesaj: **${message.content}**`)
  .setFooter("Bery Bot | Log Sistemi")
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                ...
                .setFooter(`Bery Bot | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                let embed = new Discord.MessageEmbed()
                    .setColor('#ffd100')
                .setDescription(`<a:hypesquad1:763723597863714857> ${channel.name} **Adlın Metin Kanalı  Silindi**`)
                .setFooter(`Bery Bot | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
            if (channel.type === "voice") {
...
.setTitle("SES KANALI SİLİNDİ")
                .setDescription(`<a:hypesquad1:763723597863714857> ${channel.name} **Adlı Ses Kanalı Silindi**`)
            .setFooter(`BeryBot | Log Sistemi  Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            }
    })
...
client.on("message", async message => {
  ...
  if (lus) {
    const reklamengel = ["discord.app", "discord.gg", ".party", ".com", ".az", ".net", ".io", ".gg", ".me",".com.tr", ".org", ".tr", ".gl", "glitch.me/", ".rf.gd", ".biz", ".gg", ".tk", ".tr.ht", ".ml", ".ga", ".cf", ".cq"];
      ...
      try {
        if (!newMessage.member.permissions.has('BAN_MEMBERS')) {
         newMessage.delete();
          ...
          return newMessage.channel.send(`<a:hypesquad1:763723597863714857> **Hey ${newMessage.author} Dur! Bu Sunucuda Reklamı Engelliyorum!**`).then(message => message.delete(3000));
...
client.on("guildMemberAdd", async member => {
  ...
  ///....
  if (!mesaj) {
    return client.channels.cache.get(kanal).send("<a:hypesquad1:763723597863714857> `"+ member.user.username + "`**Adlı Kullanıcı Aramıza Katıldı!** `" + sayaç + "` **Kişi Olmamıza** `" + sonuç + "` **Kişi Kaldı.** `" + member.guild.memberCount + "` **Kişiyiz!**");
  }
...
client.on("guildMemberRemove", async member => {
  ...
  if (!mesaj) {
    return client.channels.cache.get(kanal).send("<a:hypesquad1:763723597863714857> `" + member.user.username + "` **Adlı Kullanıcı Aramızdan Ayrıldı.**`" + sayaç + "` **Kişi Olmamıza** `" + sonuç + "` **Kişi Kaldı.** `" + member.guild.memberCount + "` **Kişiyiz!**");
      }
...
client.on("guildMemberAdd", async member => {
  ...
  if (!mesaj) {
    client.channels.cache.get(kanal).send("<a:hypesquad1:763723597863714857> **Selam!** `" + member.user.username + "`**.kayıtol yazarak kayıt olabilirsin!**");
